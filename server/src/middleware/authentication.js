...
const trySwitch = async (value, decoded, ownerUserId) => {
  switch (value) {
    ...
    case AUTH_MODE.isAdminOrSupervisor:
      // eslint-disable-next-line no-use-before-define
      return await checkIsAdminOrSupervisor(decoded);
    default:
...
const checkIsAdminOrSupervisor = async (decoded) => {
  try {
    const role = await getRole(decoded.role, undefined);

    if (role.name === ROLE.admin || role.name === ROLE.supervisor) return true;
    else return false;
  } catch (error) {
    logger.error(`Error checkIsAdminOrSupervisor: ${error}.`);
    throw new NotAuthorizedError();
  }
};
